...
} from "~/Page/DetailsPage/DetailsSearchTab";
import {
	DetailsTableTab,
	IDetailsTableTabProp,
} from "~/Page/DetailsPage/DetailsTableTab";
import {
	DetailsCustomTab,
	IDetailsCustomTabProp,
} from "~/Page/DetailsPage/DetailsCustomTab";
...
export function DetailsPageSubTabSwitch(props: {
	...
	const changeActiveTabkey = (key: string) => {
		...
		if (queryParams && queryParams["activeTabKey"]) {
			...
			queryParams["activeTabKey"] = __queryParams
				.slice(0, props.tabLevel + 1)
				.join("-");
	...
	useEffect(() => {
		...
		if (queryParams && queryParams["activeTabKey"]) {
			...
			const currentActiveTabkey = queryParams["activeTabKey"]
				.toString()
				.split("-")[props.tabLevel];
	...
	useEffect(() => {
		if (props.currentTabKeysInURL) {
			const _activeTabKey = props.currentTabKeysInURL.toString().split("-")[
				props.tabLevel
			];
					...
					{props.meta
						.filter((x) => {
							...
							if (
								...
								return checkInstructorApiPermission(
									(x?.tabMeta as any)?.tableProps?.searchFunc
								);
							}
							return true;
							...
							switch (x.tabType) {
								...
								case "searchtable":
									return (
										<Tabs.TabPane tab={x.tabTitle} key={i}>
											<DetailsSearchTab
												{...(x.tabMeta as IDetailsSearchTabProp)}
											/>
										</Tabs.TabPane>
									);
								case "table":
									return (
										<Tabs.TabPane tab={x.tabTitle} key={i}>
											<DetailsPageSubTabSwitch
												...
												child={
													<DetailsTableTab
														{...(x.tabMeta as IDetailsTableTabProp)}
													/>
												}
											/>
										</Tabs.TabPane>
								...
								case "custom":
									return (
										<Tabs.TabPane tab={x.tabTitle} key={i}>
											<DetailsCustomTab
												{...(x.tabMeta as IDetailsCustomTabProp)}
											/>
										</Tabs.TabPane>
									);
